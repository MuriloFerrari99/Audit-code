...
def reset() -> None:
    ...
    with admin_session() as s:
        ...
        s.execute(
            text(
                "DELETE FROM catalog_item WHERE canonical_name IN "
...
def _dt(days_ago: int) -> datetime:
    return datetime(2026, 6, 1, tzinfo=UTC) - timedelta(days=days_ago)
...
def seed() -> dict:
    ...
    with admin_session() as s:
        ...
        s.add_all(
            [
                CatalogItem(
                    id=cimento, canonical_name="Cimento CP-II 50kg", sinapi_code="1234", unit="sc"
                ),
        ...
        s.add_all(
            [
                SinapiReference(
                    sinapi_code="1234", state="PR", period="2026-05", unit="sc", price=D("30")
                ),
                SinapiReference(
                    sinapi_code="5678", state="PR", period="2026-05", unit="m3", price=D("90")
                ),
            ]
        )
    ...
    with tenant_session(str(TENANT_ID)) as s:
        ...
        s.add(
            PurchaseOrderItem(
                tenant_id=TENANT_ID,
                order_id=o1,
                catalog_item_id=cimento,
                raw_description="Cimento CP-II",
                qty=D("100"),
                unit_price=D("40"),
                total=D("4000"),
            )
        ...
        s.add(
            PurchaseOrderItem(
                ...
                order_id=o2,
                catalog_item_id=aco,
                raw_description="Aço CA-50 10mm",
                qty=D("100"),
                unit_price=D("50"),
                total=D("5000"),
            )
        ...
        s.add(
            Quotation(
                **_sourced(
                    tenant_id=TENANT_ID,
                    source_external_id="Q-1",
                    creditor_id=forn_b,
                    catalog_item_id=aco,
                    raw_description="Vergalhão 10.0",
                    qty=D("100"),
                    unit_price=D("42"),
                    valid_until=_dt(0),
                )
        ...
        for i in range(3):
            ...
            s.add(
                PurchaseOrder(
                    **_sourced(
                        tenant_id=TENANT_ID,
                        source_external_id=f"PO-FR-{i}",
                        id=oid,
                        project_id=project_id,
                        creditor_id=forn_b,
                        total=D("48000"),
                        ordered_at=_dt(25 - i * 5),
                    )
                )
            )
        ...
        s.add(
            BudgetItem(
                **_sourced(
                    tenant_id=TENANT_ID,
                    source_external_id="BUD-1",
                    project_id=project_id,
                    catalog_item_id=brita,
                    raw_description="Brita 1",
                    unit="m3",
                    qty_budgeted=D("100"),
                    qty_measured=D("130"),
                    unit_price_budgeted=D("90"),
                    total_budgeted=D("9000"),
                )
            )
        )
        ...
        s.add(
            PurchaseOrderItem(
                ...
                order_id=o6,
                catalog_item_id=brita,
                raw_description="Brita 1 - grande volume",
                qty=D("800"),
                unit_price=D("90"),
                total=D("72000"),
            )
        ...
        s.add(
            Invoice(
                **_sourced(
                    tenant_id=TENANT_ID,
                    source_external_id="INV-1",
                    order_id=o1,
                    creditor_id=forn_a,
                    number="123",
                    qty_delivered=D("100"),
                    unit_price_invoiced=D("40"),
                    total_invoiced=D("4000"),
                    issued_at=_dt(6),
                )
            )
        )
