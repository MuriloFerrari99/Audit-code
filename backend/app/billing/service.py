...
def invoice_math(plan: Plan | None, used: int, period: str) -> dict:
    """Cálculo puro da fatura: base + excedente por nota. Reusado por cliente e admin."""
    if plan is None:
        return {
            "period": period, "plan": None, "invoices_used": used,
...
def compute_monthly_invoice(session: Session, period: str | None = None) -> dict:
    ...
    return invoice_math(plan, used, p)


def suggest_upgrade(session: Session, plan: Plan | None, used: int) -> str | None:
    """Sugere o menor plano ativo que comporta o uso, quando estoura o limite atual."""
    if plan is None or used <= int(plan.invoice_limit or 0):
        return None
    nxt = session.execute(
        select(Plan)
        .where(Plan.active.is_(True), Plan.invoice_limit >= used)
        .order_by(Plan.invoice_limit.asc())
        .limit(1)
    ).scalar_one_or_none()
    return nxt.code if nxt and nxt.code != plan.code else None


def billing_summary(session: Session) -> dict:
    """Resumo p/ o painel do cliente: plano, uso, fatura projetada e sugestão de upgrade."""
    sub = _active_subscription(session)
    plan = session.get(Plan, sub.plan_id) if sub else None
    invoice = compute_monthly_invoice(session)
    invoice["subscription_status"] = sub.status if sub else "none"
    invoice["upgrade_suggested"] = suggest_upgrade(session, plan, invoice["invoices_used"])
    return invoice
